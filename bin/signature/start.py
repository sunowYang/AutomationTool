...
import os
import time
from bin.parse import ConfigRead
from download import Download
from getfileinfo import FileOperation
from result import WriteResult, ReadExcel
...
class Run:
    def __init__(self, log, base_path):
        ...
        self.data = ConfigRead(self.config_path).read()  # 解析配置文件
...
def run(log, base_path):
    ...
    :param log: 日志对象
    :param base_path: 任务文件所在路径
    :return:
    """
    _run = Run(log, base_path)
    ...
    for package_path in _run.uncompressed_paths:
        ...
        base_excel_path = base_path  # 对比excel路径
        ...
        # 3.2 对比基带版本
            ...
            row = 1
            result_path = os.path.join(base_path[:-18], 'result')
