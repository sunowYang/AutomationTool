...
class FileOperation:
    def __init__(self):
        ...
        self.tool_path = os.path.join(BASE_PATH, 'signtool.exe')
        self.info = {}
    ...
    def get_digital_signature_info(self, file_path):
        ...
        for msg in command_result.read().split('\n'):
            ...
            if 'Signature Index' in msg:
                ...
                signature_info['hash_methord'] = msg.split(' ')[3][1:-2]
            elif 'Issued to' in msg:
                signature_info['issued_to'] = msg.split(': ')[1]
            elif 'Issued by' in msg:
                signature_info['issued_by'] = msg.split(': ')[1]
            elif 'Expires' in msg:
                signature_info['expires'] = msg.split(':   ')[1]
            elif 'SHA1 hash:' in msg:
                signature_info['sha1_hash'] = msg.split(': ')[1]
        # 加入最后一个签名信息
        signature_list.append(signature_info)
